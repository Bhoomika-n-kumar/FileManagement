...
export class AdfolderComponent implements OnInit {
  ...
  /* not needed */
  getAllUser() {
    this.logSer.getAllUser().subscribe((data: Login[]) => {
      this.allUser = data;
  ...
  getFolderNames() {
    this.logSer.getFolder().subscribe((data: Folder[]) => {
      (this.allFolder = data), console.table(this.allFolder);
      /*this.folderSet.add(this.allFolder);
        console.log(this.folderSet)*/
    });
  }
  ...
  deleteEmployee(id: number) {
    console.log(id);
    this.logSer.deleteEmployee(id).subscribe((data: Folder) => {
      this.getFolderNames();
    });
  }
  ...
  initial() {
    document
      ...
      .setAttribute(
        "style",
        "border-style: dotted; width:90px; margin-left:20px;height:67px; margin-top:10px;float:left;"
      );
  }
  ...
  listView() {
    ...
    for (i = 0; i < elements.length; i++) {
      elements[i].setAttribute(
        "style",
        "outline:none; border:none; cursr: pointer; margin-left:20px; margin-top:10px; height :30px ; display:block; background-color:#d1dade;width:1000px;text-align:left; padding:5px;"
      );
    }
  }
  ...
  gridView() {
    ...
    for (i = 0; i < elements.length; i++) {
      elements[i].setAttribute(
        "style",
        "outline:none; border:none; cursr: pointer; margin-left:20px; margin-top:10px; height :67px ; display:block;float:left; background-color:#d1dade;width:90px;text-align:center; padding:5px;"
      );
    }
  }
  ...
  originalView() {
    ...
    for (i = 0; i < elements.length; i++) {
      elements[i].setAttribute(
        "style",
        'outline:none; border:none; cursor: pointer; margin-top:20px; margin-left:20px; height :30px ;  width:90px; background-color:#d1dade;"'
      );
    }
  }
  ...
  myFunction() {
    ...
    if (person == null || person == "") {
      ...
      button.setAttribute(
        "style",
        "outline:none; border:none; cursr: pointer; margin-left:20px; margin-top:20px; height :30px ; display:block; width:90px; background-color:#d1dade;"
      );
